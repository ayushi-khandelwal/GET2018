...
public class JobScheduler
{
	public static void main(String args[])
	{
		...
		System.out.println("\nTURN AROUND TIME :-");	
		for(int i=0 ; i<noOfProcess ; i++)	//computing turn around time
		{
		...
		System.out.println("\nWAITING TIME :-");	
		for(int i=0 ; i<noOfProcess ; i++)	//computing waiting time
		{
			waitingTime[i] = turnAroundTime[i] - burstTime[i];
			System.out.println ("The waiting time for process " + (i+1) + " is " + waitingTime[i]);
			totalWaitingTime += waitingTime[i];
		}
		avgWaitingTime = totalWaitingTime / noOfProcess;	//computing average waiting time
